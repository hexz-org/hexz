//! Unified error type and result alias for Strata.
//!
//! Defines `StrataError` and the crate-wide `Result<T>` alias so that all
//! library and CLI code can report I/O, format, compression, and
//! encryption failures through a single type.

use thiserror::Error;

/// Unified error type for all Strata library and CLI operations.
///
...
/// Convenience result alias for functions that can fail with `StrataError`.
///
...
/// Strata errors rather than arbitrary `std` errors.
///
/// **Constraints:** Callers should treat `StrataError` as opaque and avoid
