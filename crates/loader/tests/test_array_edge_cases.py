...
def test_read_array_reader_instance_cleanup(tmp_path):
    ...
    result1 = strata.read_array(str(snap_path), shape=(100,), dtype="float32")
    assert np.array_equal(result1, arr)

    # Pass Reader instance (should NOT auto-close)
    reader = strata.open(str(snap_path), mode="r")
    result2 = strata.read_array(reader, shape=(100,), dtype="float32")
...
def test_write_array_writer_instance_cleanup(tmp_path):
    ...
    # Verify written
    result = strata.read_array(str(snap_path), shape=(100,), dtype="float32")
...
def test_write_array_contiguous_check(tmp_path):
    ...
    result = strata.read_array(str(snap_path), shape=(10, 10), dtype="float32")
    assert np.array_equal(result, arr)
...
def test_arrayview_import_check(tmp_path):
    ...
    try:
        with pytest.raises(ImportError, match="NumPy is required"):
            strata.ArrayView("dummy.st", shape=(10,), dtype="float32")
    finally:
        # Restore
        strata.array.HAS_NUMPY = original_has_numpy
...
def test_arrayview_integer_index_multidim(tmp_path):
    ...
    view = strata.ArrayView(str(snap_path), shape=(10, 10, 10), dtype="int32")
...
def test_arrayview_slice_1d_full(tmp_path):
    ...
    snap_path = tmp_path / "1d.st"
    arr = np.arange(100, dtype=np.float32)
    strata.write_array(str(snap_path), arr)

    view = strata.ArrayView(str(snap_path), shape=(100,), dtype="float32")
...
def test_arrayview_slice_2d_rows(tmp_path):
    ...
    snap_path = tmp_path / "2d.st"
    arr = np.arange(1000, dtype=np.float32).reshape(100, 10)
    strata.write_array(str(snap_path), arr)

    view = strata.ArrayView(str(snap_path), shape=(100, 10), dtype="float32")
...
def test_read_array_dtype_conversion(tmp_path):
    ...
    # String dtype
    result1 = strata.read_array(str(snap_path), shape=(100,), dtype="int32")
    assert result1.dtype == np.int32

    # NumPy dtype object
    result2 = strata.read_array(str(snap_path), shape=(100,), dtype=np.dtype("int32"))
...
def test_write_array_tobytes_conversion(tmp_path):
    ...
    for shape in [(100,), (10, 10), (5, 4, 5)]:
        ...
        # Verify size
        result = strata.read_array(str(snap_path), shape=shape, dtype="float32")
        assert np.array_equal(result, arr)
...
def test_arrayview_itemsize_calculation(tmp_path):
    ...
    view = strata.ArrayView(str(snap_path), shape=(100,), dtype="float64")
...
def test_arrayview_multidim_row_size(tmp_path):
    ...
    view = strata.ArrayView(str(snap_path), shape=(20, 10, 10), dtype="int32")
...
def test_read_array_size_calculation(tmp_path):
    ...
    for shape, dtype, expected_size in test_cases:
        ...
        result = strata.read_array(str(snap_path), shape=shape, dtype=dtype)
...
def test_arrayview_with_different_offsets(tmp_path):
    ...
    view1 = strata.ArrayView(str(snap_path), shape=(100,), dtype="float32", offset=0)
    assert np.array_equal(view1[:], arr1)

    view2 = strata.ArrayView(str(snap_path), shape=(100,), dtype="float32", offset=400)
    assert np.array_equal(view2[:], arr2)

    view3 = strata.ArrayView(str(snap_path), shape=(100,), dtype="float32", offset=800)
