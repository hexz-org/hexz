...
pub fn run(
    ...
    println!(
        "{} Size         {}",
        "│".dimmed(),
        HumanBytes(report.file_size).to_string().green()
    );
    println!(
        "{} Block Size   {}",
        "╰".dimmed(),
        HumanBytes(report.block_size as u64)
            .to_string()
            .bright_black()
    );
    ...
    println!(
        "    {} Zero Blocks    {:.1}%",
        "→".dimmed(),
        report.zero_block_pct * 100.0
    );
    println!(
        "    {} Mean Entropy   {:.2} bits/byte",
        "→".dimmed(),
        report.mean_entropy
    );
    println!(
