...
//!
//! This module provides commands for managing virtual machines backed by Strata
...
//!   - Packs result into Strata archive
//!
//! ## Snapshot Management
//!
...
//! strata vm install --iso ubuntu.iso --disk-size 20G --output ubuntu.st
//!
//! # 2. Boot with persistent overlay
//! strata vm boot ubuntu.st --persist overlay.bin
//!
//! # (Make changes in VM, then shut down)
//!
//! # 3. Commit changes to new snapshot
//! strata vm commit ubuntu.st overlay.bin ubuntu-v2.st --message "Installed updates"
//!
//! # 4. Boot new snapshot
//! strata vm boot ubuntu-v2.st
//! ```
//!
...
//! strata vm boot ubuntu.st --qmp-socket /tmp/qmp.sock --persist overlay.bin
//!
//! # Create live snapshot (VM keeps running)
//! strata vm snap --socket /tmp/qmp.sock --base ubuntu.st \
...
//!
//! ```bash
//! strata vm commit base.st overlay.bin delta.st --thin
