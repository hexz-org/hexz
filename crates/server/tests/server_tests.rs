...
fn test_parse_range_valid_unbounded() {
    assert_eq!(parse_range("bytes=1024-", 10000), Ok((1024, 9999)));
}

#[test]
fn test_parse_range_single_byte() {
    assert_eq!(parse_range("bytes=0-0", 1000), Ok((0, 0)));
}

#[test]
fn test_parse_range_at_eof() {
    // bytes=0-999 with size=1000 means last valid byte is 999
    assert_eq!(parse_range("bytes=0-999", 1000), Ok((0, 999)));
}

#[test]
fn test_parse_range_missing_prefix() {
    assert_eq!(parse_range("0-1023", 10000), Err(()));
}

#[test]
fn test_parse_range_invalid_integers() {
    assert_eq!(parse_range("bytes=abc-def", 10000), Err(()));
}

#[test]
fn test_parse_range_inverted_range() {
    assert_eq!(parse_range("bytes=1000-500", 10000), Err(()));
}

#[test]
fn test_parse_range_out_of_bounds() {
    // size=1000, valid range is 0-999; requesting 0-1000 is out of bounds
    assert_eq!(parse_range("bytes=0-1000", 1000), Err(()));
}

#[test]
fn test_parse_range_suffix_unsupported() {
    assert_eq!(parse_range("bytes=-500", 10000), Err(()));
}

#[test]
fn test_parse_range_empty_after_prefix() {
    assert_eq!(parse_range("bytes=", 10000), Err(()));
}
...
fn test_parse_range_constants() {
    ...
    assert_eq!(parse_range("bytes=0-1023", large_size), Ok((0, 1023)));
}

#[test]
fn test_parse_range_mid_range() {
    assert_eq!(parse_range("bytes=500-1499", 5000), Ok((500, 1499)));
}
